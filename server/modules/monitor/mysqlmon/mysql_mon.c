...
void* info_copy_func(const void *val)
{
    ...
    MYSQL_SERVER_INFO *new_val = malloc(sizeof(MYSQL_SERVER_INFO));
    char *binlog_name = strdup(old_val->binlog_name);
 ...
 */
static void *
startMonitor(void *mon, void* parameters)
{
    CONFIG_PARAMETER *params = (CONFIG_PARAMETER*)parameters;
    ...
    else
    {
        ...
        HASHTABLE *server_info = hashtable_alloc(MAX_NUM_SLAVES, (int(*)())simple_str_hash, (int(*)())strcmp);
        ...
        hashtable_memory_fns(server_info, (HASHMEMORYFN)strdup, (HASHMEMORYFN)info_copy_func,
                             (HASHMEMORYFN)free, (HASHMEMORYFN)info_free_func);
 ...
 */
static void diagnostics(DCB *dcb, void *data)
{
    MONITOR *mon = (MONITOR*)data;
            ...
            do
            {
                ...
                if (serv_info->slave_io && serv_info->slave_sql)
                {
                    if (nrunning == 0)
                    {
                        ...
                        char *binlog_name = strdup(row[i_binlog_name]);

                        if (binlog_name)
                        {
                            free(serv_info->binlog_name);

...
value of any variables used in the select it **not** considered. For instance,
if a variable is used in the _WHERE_ clause of the select, then a subsequent
...
configured who the caching should be apply to, the presence of the cache
...
but the user _bob_ does not. If _bob_ tries to access he will get an error
as the reply:
```
...
a select after _alice_ has done so.
...
+------+------+
```
That can be prevented, by explcitly declaring in the rules that the caching

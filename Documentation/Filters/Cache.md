...
Note that in order to function the cache needs all statements to be parsed.
The implication of that is that unless statements _already_ need to be parsed,
...
The cache needs to parse all statements
...
of the cached data is 100 mebibytes. The rules for the cache are in the file
`cache_rules.json`.
